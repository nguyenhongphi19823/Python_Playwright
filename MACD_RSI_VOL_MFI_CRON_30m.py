...
# Fetch latest 500 candles (ensuring recent data)
def fetch_data(symbol, timeframe='30m'):
    try:
        ohlcv = exchange.fetch_ohlcv(symbol, timeframe, limit=500)  # Fetch latest candles only
...
def calculate_mfi(df, period=14):
    ...
    mfi = 100 * (sum_positive_flow / (sum_positive_flow + sum_negative_flow + 1e-10))  # Avoid div by zero
    df['mfi'] = mfi
    return df
...
def check_conditions(df):
    ...
    # 🟢 Print real-time debugging info
    ...
    conditions = [
        # Condition 1: MACD above signal, RSI & MFI within range, increasing volume
        latest['macd'] > latest['signal'] and
        40 < latest['rsi'] < 80 and
        40 < latest['mfi'] < 80 and
        latest['volume'] > latest['mavol'] and
        previous['volume'] > previous['mavol'],

        # Condition 2: MACD crossover, RSI & MFI rising from oversold, increasing volume
        latest['macd'] > latest['signal'] and
...
if opportunities:
    ...
    # Send email notification
    subject = "🚀 Crypto Trading Alert: Potential Opportunities for MACD_RSI_VOL_MFI_30m"
